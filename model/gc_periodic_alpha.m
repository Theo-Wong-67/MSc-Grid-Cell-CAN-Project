function [pos_x, pos_y, rm_smooth, sNeurons, rm_raw, dmg] = ...
    gc_periodic_alpha(n, tau, dt, beta, alphabar, abar, wtphase, steps, eta_0, alpha, R)

% GC_PERIODIC_ALPHA  CAN grid-cell model with Zhi & Cox (2021) damage.
...
% Parameter in correspondence with Zhi & Cox (2021):
%   alpha    — Synaptic scaling
%   R        — Damaged radius
...
%   dmg         — metrics related to damaged region
    ...
    % ---- Zhi & Cox damage mask: centred disk of radius R ----
    [rr_g, cc_g] = ndgrid(1:n, 1:n);
    centre = [n/2, n/2];   % centred damage index
    is_dmg = hypot(rr_g-centre(1), cc_g-centre(2)) <= R; % Damaged region specification
    if R <= 0, is_dmg = false(n,n); end
    alpha_mask = ones(n,n);
    alpha_mask(is_dmg) = alpha;
    dmg = struct('alpha',alpha, 'R',R, 'centre',centre, 'mask',alpha_mask, ...
                 'is_damaged',is_dmg, 'n_damaged',nnz(is_dmg), 'fraction',nnz(is_dmg)/n^2, ...
                 'idx_damaged',find(is_dmg(:)), 'idx_healthy',find(~is_dmg(:))); % store usable parameters

    % Initiation phase (HEALTHY -- damage applied after, per Zhi & Cox)
    for iter = 1:1000 
        if iter == 800
    ...
    for iter = 1:steps -20 % -20 from Burak and Fiete
        ...
        % Small convolution -- Zhi & Cox: attenuate OUTPUT of damaged neurons
        r_out = alpha_mask .* r;
        convolution = real(ifft2( ...
        fft2(r_out.*typeR).*ftr_small + ...
        fft2(r_out.*typeL).*ftl_small + ...
        fft2(r_out.*typeD).*ftd_small + ...
        fft2(r_out.*typeU).*ftu_small));
