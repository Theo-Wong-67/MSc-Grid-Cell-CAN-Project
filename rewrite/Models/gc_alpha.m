function [rm_raw, rm_smooth, dmg] = ...
    gc_alpha(n, tau, dt, beta, alphabar, abar, wtphase, steps, eta_0, a, R)
...
%   rm_raw      — n^2 unsmoothed rate maps (n_bins × n_bins)
    ...
    for iter = 1:steps -20
        ...
        rfield = venvelope .* ((1 + eta_0*vel*right)*typeR + (1 + eta_0*vel*left)*typeL + ...
                           (1 + eta_0*vel*up)*typeU   + (1 + eta_0*vel*down)*typeD); % Equation 4 Burak & Fiete 2009
        % Small convolution, same implementations as initiation phase
        r_out = a_mask .* r;                        % Damage application
