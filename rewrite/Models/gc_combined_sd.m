function [rm_raw, rm_smooth, dmg] = ...
    gc_combined_sd(n, tau, dt, beta, alphabar, abar, wtphase, steps, eta_0, a, R, sigma_v, mu_v)
% GC_COMBINED_SD  Synaptic damage + SIGNAL-DEPENDENT afferent noise.
...
%   mu_v     — reference mean speed (m/s), default 0.4998. Sets what sigma_v
%              MEANS under the signal-dependent law; not a dynamical parameter.
...
%   dmg         — damage mask and bookkeeping, as gc_alpha
    ...
    % Skip the per-timestep draw entirely when there is no noise. 0*randn is
    % still 0, but at 400k steps x n^2 that is ~6.5e9 wasted gaussians, and
    % sigma_v = 0 is a whole column of the combined sweep. Nothing downstream
    % of the walk consumes the stream, so this does not change any result.
    if nargin < 13 || isempty(mu_v), mu_v = 0.4998; end   % measured mean speed, m/s
    assert(mu_v > 0, 'gc_combined_sd:mu_v', 'mu_v must be positive');
    ...
    for iter = 1:steps -20
        ...
        if do_noise, E = sigma_v*sqrt(vel/mu_v)*randn(n,n); end   % sigma(t)=sigma_v*sqrt(|v|/mu_v)
