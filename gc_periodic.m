...
% DAMAGE MASK
R_damage = 7;          % damage radius in neurons
alpha_damage = 0.1;    % synaptic output strength
damage_mask = ones(n,n);
centre = [n/2, n/2];
for i = 1:n
    for j = 1:n
        if sqrt((i-centre(1))^2 + (j-centre(2))^2) <= R_damage
            damage_mask(i,j) = alpha_damage;
        end
    end
end
...
vel=0; 
    ...
    for iter=1: sampling_length  - 20      
        ...
        % Equation (4) 
        rfield = damage_mask.*venvelope.*((1+alpha*vel*right)*typeR+(1+alpha*vel*left)*typeL+(1+alpha*vel*up)*typeU+(1+alpha*vel*down)*typeD);   
        
        % Convolute population activity with shifted semmetric weights.
        % real() is implemented for octave compatibility
        r_damaged = r .* damage_mask;

        convolution = real(ifft2( ...
                  fft2(r_damaged.*typeR).*ftr_small ...
                + fft2(r_damaged.*typeL).*ftl_small ...
                + fft2(r_damaged.*typeD).*ftd_small ... 
                + fft2(r_damaged.*typeU).*ftu_small));

        disp(min(damage_mask(:)));  % should print 0.3 if mask is correct
        disp(max(damage_mask(:)));  % should print 1
        disp(sum(damage_mask(:) < 1)); % should print number of damaged neurons
        disp(min(r_damaged(:)));
        disp(max(r_damaged(:)));
